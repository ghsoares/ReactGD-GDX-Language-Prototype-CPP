...
ImportToken *GDXLexer::import()
{
	...
	if (found_match())
	{
		...
		expect_next("Expected name of the imported component");
		T_SYMBOL();

		std::string class_name = *get_str(-1);

		expect_next("Expected token \"from\"");
		match("from");
...
VariableToken *GDXLexer::variable(bool require_prefix)
{
...
FunctionToken *GDXLexer::function()
{
...
TagToken *GDXLexer::tag()
{
	...
	if (found_match())
	{
		...
		std::map<std::string, std::string> props = tag_properties();
		...
		else
		{
			...
			{
				throw new ParseException("Can't end closing tag with \"/>\"", tag_close_start);
			}
		}
...
std::map<std::string, std::string> GDXLexer::tag_properties()
{
	...
	while (!cursor()->eof)
	{
		...
		if (found_match())
		{
			...
			std::string prop_value = *get_str(-1);
...
void GDXLexer::T_SYMBOL(bool prop)
{
	if (prop)
	{
...
void GDXLexer::T_STRING(bool multiline)
{
	open_match();
...
void GDXLexer::T_GDBLOCK()
{
	...
	if (found_match())
	{
		...
		if (lvl >= 0)
		{
			throw new ParseException("Couldn't find GD block end", cursor());
		}
